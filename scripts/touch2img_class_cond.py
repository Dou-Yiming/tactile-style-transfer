...
def main():
    ...
    parser.add_argument(
        ...
        default="outputs/touch2img_class-condition/"
    )
    parser.add_argument(
        "--skip_grid",
        action='store_true',
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/touch/eval_touch2img/touch_and_go/class_condition.yaml",
        help="path to config which constructs model",
    )
    parser.add_argument(
        "--ckpt",
        type=str,
        default="logs/2023-02-12T13-21-58_touch2img_class_condition_seg_mask/checkpoints/last.ckpt",
                ...
                # for i, one_data in enumerate(data.datasets['test']):
                    ...
                    for n in trange(opt.n_iter, desc="Sampling"):
                        for prompts in tqdm(prompts_lists, desc="within_datapoint"):
                            ...
                            # print(prompts)
                            prompts = prompts.repeat(opt.n_samples)
                            prompts = {'class_label': prompts}
                            # exit()
                            # prompts = list(prompts)
                    ...
                    if not opt.skip_grid:
                        ...
                        # 把原图和gelsight放进去
                        origin_image = torch.cat((map_back(one_data[model.first_stage_key]), map_back(one_data["aux"])), 0)
