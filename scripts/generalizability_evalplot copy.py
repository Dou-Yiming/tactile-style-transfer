...
if __name__ == '__main__':
    ...
    #         pred_gan_feat = tac_enc(pred_gan.unsqueeze(0)).squeeze(0)
    #         pred_l1_feat = tac_enc(pred_l1.unsqueeze(0)).squeeze(0)
    ...
    #                 torch.dot(input_feat, pred_gan_feat).cpu().detach().numpy(),
    #                 torch.dot(input_feat, pred_l1_feat).cpu().detach().numpy()
    #             ]
    #         ]
    #     )
    ...
    score_bin = [list(np.array(bin).mean(0)) for bin in score_bin]
    # Create a DataFrame
    data = {
        'Diffusion': score_bin[0],
        'GAN': score_bin[1],
        'L1': score_bin[2],
    }
    ...
    # Rename the axes
    plt.xlabel('')
    plt.ylabel('CVTP Score', fontsize=14)
