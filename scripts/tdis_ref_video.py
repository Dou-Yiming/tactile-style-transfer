...
def main():
    ...
    parser.add_argument(
        ...
        default="outputs/tdis_ref-sample"
    )
    parser.add_argument(
        "--skip_grid",
        action='store_true',
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/touch/eval_ref2img/ref2img_cmc_video_trainable_seg_mask_hybrid_noflip_tdis.yaml",
        help="path to config which constructs model",
    )
    parser.add_argument(
        "--ckpt",
        type=str,
        default="logs/2023-02-15T11-56-01_ref2img_cmc_video_trainable_seg_mask_hybrid_noflip/checkpoints/last.ckpt",
        ...
        with precision_scope("cuda"):
            with model.ema_scope():
                for i in trange(len(data.datasets['test']), desc="Data"):
                    ...
                    prompts_lists = [one_data['aux'].to(device)]
                    ...
                    for n in trange(opt.n_iter, desc="Sampling"):
                        for prompts in tqdm(prompts_lists, desc="within_datapoint"):
                            ...
                            if not opt.skip_save:
                                ...
                                origin_images = [map_back(one_data["imageA"]), map_back(one_data["reflectance"]), map_back(one_data["imageB"])]
                                origin_name = ['imageA', 'reflectance', 'imageB']

                                # append gelsight
                                gelsight_images = one_data["aux"][:,3:,:,:]
                                reference_length = int(gelsight_images.shape[1] / 3)
                                
                                for ref in range(reference_length):
                                    origin_images.append(map_back(gelsight_images[:,3 * ref:3 * ref + 3]))
                    ...
                    if not opt.skip_grid:
                        ...
                        origin_image = [map_back(one_data["imageA"]), map_back(one_data['reflectance']), map_back(one_data["imageB"])]
                        # print(len(origin_images))
                        # exit()

                        # append gelsight
                        # prin(one_data["aux"].shape)
                        gelsight_images = one_data["aux"][:,3:,:,:]
                        reference_length = int(gelsight_images.shape[1] / 3)

                        for ref in range(reference_length):
                            # print(gelsight_images[:,3 * ref:3 * ref + 3, :, :].shape)
                            origin_image.append(map_back(gelsight_images[:,3 * ref:3 * ref + 3, :, :]))
                        # print(len(origin_images))
                        origin_image = torch.cat(origin_image, 0)
                        ...
                        # n_rows = opt.n_rows if opt.n_rows > 0 else (batch_size + reference_length + 3)
