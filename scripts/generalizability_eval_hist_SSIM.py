...
if __name__ == '__main__':
    ...
    for instance_idx, instance in tqdm(enumerate(split['test']),total=len(split['test']), leave=False):
        ...
        dist_score_pairs.append(
            [
                dist,
                [
                    calc_ssim(pred_diffusion.unsqueeze(0), gt.unsqueeze(0)).squeeze().cpu().detach().numpy(),
                    calc_ssim(pred_gan.unsqueeze(0), gt.unsqueeze(0)).squeeze().cpu().detach().numpy(),
                    calc_ssim(pred_l1.unsqueeze(0), gt.unsqueeze(0)).squeeze().cpu().detach().numpy(),
                ]
            ]
        )
    ...
    plt.savefig('SSIM_hist.png', dpi=300, bbox_inches='tight')
