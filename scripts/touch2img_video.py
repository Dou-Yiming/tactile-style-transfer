...
def main():
    ...
    parser.add_argument(
        ...
        default="outputs/touch2img_cmc_video_reference5_trainable_no_ckpt_seg_mask_noflip-sample10000/"
    )
    parser.add_argument(
        "--skip_grid",
        action='store_true',
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/touch/eval_touch2img/touch_and_go/touch2img_cmc_video_reference5_trainable_seg_mask_no_ckpt_noflip.yaml",
        help="path to config which constructs model",
    )
    parser.add_argument(
        "--ckpt",
        type=str,
        default="logs/2023-05-24T05-58-05_touch2img_cmc_video_reference5_trainable_no_ckpt_seg_mask_noflip/checkpoints/last.ckpt",
    ...
    with torch.no_grad():
        with precision_scope("cuda"):
            with model.ema_scope():
                ...
                # for i, one_data in enumerate(data.datasets['test']):
                    tic = time.time()
                    all_samples = list()
                    ...
                    for n in trange(opt.n_iter, desc="Sampling"):
                        for prompts in tqdm(prompts_lists, desc="within_datapoint"):
                            ...
                            samples_ddim, _ = sampler.sample(S=opt.ddim_steps,
                                                            ...
                                                            eta=opt.ddim_eta,
                                                            x_T=start_code)
                            # samples_ddim, _ =sampler.sample(opt.ddim_steps,opt.n_samples,
                            ...
                            if not opt.skip_save:
                                ...
                                origin_images = [map_back(one_data["image"])]
                                origin_name = ['reference_image']
                                reference_length = int(one_data["aux"].shape[1] / 3)

                                for ref in range(reference_length):
                                    origin_images.append(map_back(one_data["aux"][:,3 * ref:3 * ref + 3]))
                                    origin_name.append('gelsight_{}'.format(ref))

                                # origin_images = [map_back(one_data["image"]), map_back(one_data["aux"])]
                    ...
                    if not opt.skip_grid:
                        ...
                        # 把原图和gelsight放进去
                        origin_images = [map_back(one_data[model.first_stage_key])]
                        origin_name = ['reference_image']
                        reference_length = int(one_data[model.cond_stage_key].shape[1] / 3)

                        for ref in range(reference_length):
                            origin_images.append(map_back(one_data[model.cond_stage_key][:,3 * ref:3 * ref + 3]))
                            origin_name.append('gelsight_{}'.format(ref))
                        
                        origin_image = torch.cat(origin_images, 0)
                        ...
                        n_rows = opt.n_rows if opt.n_rows > 0 else (batch_size + reference_length + 1)
