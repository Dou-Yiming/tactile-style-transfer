...
def main():
    ...
    parser.add_argument(
        ...
        default="outputs/visgel-img2touch-final-all/"
    )
    parser.add_argument(
        "--skip_grid",
        action='store_true',
    ...
    parser.add_argument(
        "--config",
        type=str,
        default="configs/touch/eval_img2touch/visgel_img2touch.yaml",
        help="path to config which constructs model",
    )
    parser.add_argument(
        "--ckpt",
        type=str,
        default="logs/2023-02-27T14-26-34_visgel_image2touch_concat_all_full/checkpoints/last.ckpt",
            ...
            with model.ema_scope():
                ...
                # for i, one_data in enumerate(data.datasets['test']):
                    ...
                    for n in trange(opt.n_iter, desc="Sampling"):
                        for prompts in tqdm(prompts_lists, desc="within_datapoint"):
                            ...
                            if not opt.skip_save:
                                ...
                                origin_images = [map_back(one_data["image"]), map_back(one_data[model.cond_stage_key])[:,3:,:,:]]
                                origin_name = ['gelsight_gt', 'vision']
                    ...
                    if not opt.skip_grid:
                        ...
                        # 把原图和gelsight放进去
                        origin_image = torch.cat((map_back(one_data['image']), map_back(one_data[model.cond_stage_key])[:,3:,:,:]), 0)
